    ...
    <main id="top">
      <section className="auction-hero" aria-labelledby="hero-title">
        <div className="hero-copy">
          <span className="eyebrow"><Sparkles size={14} /> Verified dealer auctions / Lagos and Abuja</span>
          <h1 id="hero-title">A normal auto auction.<br /><em>Your maximum stays private.</em></h1>
          <p className="hero-lede">Inspect the vehicle, review the terms and enter your maximum in Naira. While the timed auction is open, no dealer can see your amount, rank or identity.</p>
        ...
        <article className="hero-lot" aria-label={`Featured lot ${featuredAuction.title}`}>
          ...
          <div className="hero-lot-body"><span className="lot-kicker">Featured dealer lot / {featuredAuction.id}</span><h2>{featuredAuction.title}</h2><p>{featuredAuction.subtitle}</p><div className="hero-lot-meta"><span><MapPin size={14} />{featuredAuction.location}</span><span><Clock3 size={14} />{featuredAuction.endsIn}</span></div><div className="hero-price"><span>Opening offer</span><strong>{formatNaira(featuredAuction.openingBidNaira)}</strong><small>{featuredAuction.sealedOfferCount} confidential offers / no price ladder</small></div><button className="card-action" onClick={() => openLot(featuredAuction)}>Inspect lot and offer <ArrowRight size={15} /></button></div>
        </article>
      </section>
      ...
      <section className="lots-section" id="auctions" aria-labelledby="lots-title">
        <div className="section-heading"><div><span className="eyebrow">Live inventory</span><h2 id="lots-title">Review the lot. Set your number. Keep it off the board.</h2></div><p>Every lot shows condition, location, starting offer, increment and close time. The bid ladder is the only thing removed.</p></div>
...
function BidPanel({ auction, session, sealedOffer, bidValue, bidError, isOpeningSoon, onBidChange, onSealOffer, onStartOffer }: { auction: Auction; session: DealerSession | null; sealedOffer?: bigint; bidValue: string; bidError: string; isOpeningSoon: boolean; onBidChange: (value: string) => void; onSealOffer: () => void; onStartOffer: () => void }) {
  ...
  if (!session) return <section className="bid-panel"><div><span className="eyebrow">Dealer verification</span><h3>Sign in before you offer</h3><p>Use normal dealership details. No crypto wallet is required.</p></div><button className="drawer-primary" onClick={onStartOffer}><UserRound size={17} />Sign in to place a private offer</button></section>;
  if (sealedOffer !== undefined) return <section className="bid-panel sealed"><CheckCircle2 size={20} /><div><span className="eyebrow">Your offer status</span><h3>Private maximum prepared</h3><strong>{formatNaira(sealedOffer / KOBO_PER_NAIRA)}</strong><p>Only {session.dealerName} can see this amount. Other dealers see only the confidential-offer count.</p></div></section>;
...
function DealerSessionModal({ onClose, onSubmit }: { onClose: () => void; onSubmit: (input: { dealerName: string; businessName: string; contact: string }) => void }) {
  ...
  return <div className="session-backdrop" onClick={onClose}><form className="session-modal" onSubmit={submit} onClick={(event) => event.stopPropagation()}><button type="button" className="drawer-close" onClick={onClose} aria-label="Close dealer sign-in"><X size={18} /></button><div className="session-icon"><UserRound size={20} /></div><span className="eyebrow">Dealer access</span><h2>Enter the auction without a wallet.</h2><p>OjaBid is built around dealer verification and Naira offers, not crypto language. Your dealer identity is not shown in an open auction.</p><label>Dealer name<input required value={dealerName} onChange={(event) => setDealerName(event.target.value)} placeholder="e.g. Ayomide D." /></label><label>Dealership<input required value={businessName} onChange={(event) => setBusinessName(event.target.value)} placeholder="e.g. Dami Autos" /></label><label>Phone or email<input required value={contact} onChange={(event) => setContact(event.target.value)} placeholder="Your contact" /></label><button className="drawer-primary" type="submit"><UserRound size={17} />Continue as dealer</button><small className="session-note">Dealer verification is required before a confidential offer is accepted.</small></form></div>;
}
