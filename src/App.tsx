...
import { hasConfiguredDeployment } from "./lib/contract-config";
import { registerBidderForAuction, submitEncryptedBid } from "./lib/fhevm-bid";
import { fetchPublicAuctionStates, type PublicAuctionState } from "./lib/auction-chain";
...
function Badge({ status }: { status: AuctionStatus }) {
  const className = status === "Open" ? "open" : status === "Closing soon" ? "closing" : status === "Closed" ? "closed" : "soon";
...
function App() {
  ...
  const [relayTokens, setRelayTokens] = useState<Record<string, string>>({});
  const [accessCode, setAccessCode] = useState("");
  const [bidInFlight, setBidInFlight] = useState(false);
  const [bidError, setBidError] = useState("");
  const [onchainStates, setOnchainStates] = useState<Map<number, PublicAuctionState>>(new Map());

  useEffect(() => {
    if (!hasConfiguredDeployment) return;
    let cancelled = false;
    const refresh = async () => {
      try {
        const states = await fetchPublicAuctionStates(auctions.map((auction) => auction.onchainAuctionId));
        if (!cancelled) setOnchainStates(states);
      } catch {
        // Keep the last verified on-chain state visible. The bid API remains the final authority.
      }
    };
    void refresh();
  ...
  const displayAuctions = useMemo(() => auctions.map((auction) => {
    const state = onchainStates.get(auction.onchainAuctionId);
    return state ? { ...auction, ...state } : auction;
  }), [onchainStates]);
  const selectedAuction = selectedAuctionId ? displayAuctions.find((auction) => auction.id === selectedAuctionId) ?? null : null;

  const filtered = useMemo(() => displayAuctions.filter((auction) => {
  ...
  }), [audience, category, displayAuctions, query]);

  const savedAuctions = displayAuctions.filter((auction) => saved.includes(auction.id));
  const offeredAuctions = session ? displayAuctions.filter((auction) => sealedOffers[auction.id]?.bidderId === session.sessionId) : [];
  const featuredAuction = filtered[0] ?? displayAuctions[0];
  ...
  async function registerSessionForAuction(auction: Auction, bidder: BidderSession, code: string) {
    if (!hasConfiguredDeployment) return "";
    if (relayTokens[auction.id]) return relayTokens[auction.id];
    if (!code.trim()) throw new Error("Enter the auction access code supplied after account verification.");
    const registration = await registerBidderForAuction({ auctionId: auction.onchainAuctionId, bidderId: bidder.bidderId, accessCode: code.trim() });
    setRelayTokens((current) => ({ ...current, [auction.id]: registration.accessToken }));
  ...
  function startOffer(auction: Auction) {
    ...
    if (auction.status === "Opening soon") {
      if (!hasConfiguredDeployment) {
        setBidError("Encrypted bidding is not configured yet. A live Sepolia contract and relay must be set before registration can begin.");
        return;
      }
      void registerSessionForAuction(auction, session, accessCode)
        .then(() => setBidError("Your account is approved for this lot. Offers open at the published time."))
        .catch((error) => setBidError(error instanceof Error ? error.message : "Could not complete auction registration."));
      return;
    }
    if (auction.status === "Closed") {
      setBidError("This auction has closed and is no longer accepting confidential offers.");
      return;
    }
    setBidError("");
  }

  async function handleSession(input: { name: string; organisation?: string; contact: string; accessCode?: string }) {
    setSessionError("");
    try {
      const bidder = createBidderSession({ ...input, audience: sessionIntent });
      setSession(bidder);
      setAccessCode(input.accessCode ?? "");
      setSessionOpen(false);
      if (hasConfiguredDeployment && selectedAuction?.status === "Opening soon") {
        await registerSessionForAuction(selectedAuction, bidder, input.accessCode ?? "");
        setBidError("Your account is approved for this lot. Offers open at the published time.");
      }
    } catch (error) {
  ...
  async function sealOffer(auction: Auction) {
    if (!session || session.audience !== auction.audience) {
      openSession(auction.audience);
      return;
    }
    ...
    try {
      setBidInFlight(true);
      if (hasConfiguredDeployment) {
        let token = relayTokens[auction.id];
        if (!token) {
          token = await registerSessionForAuction(auction, session, accessCode);
        }
        if (!token) throw new Error("Your account must be approved before this auction opens. Complete registration while the lot is opening soon.");
        await submitEncryptedBid({ auctionId: auction.onchainAuctionId, bidderId: session.bidderId, accessToken: token, amountKobo: kobo });
      } else {
      ...
      setBidError(error instanceof Error ? error.message : "Your encrypted maximum could not be submitted.");
    } finally {
  ...
  return <div className="site-shell">
    ...
    {selectedAuction && <AuctionDrawer key={selectedAuction.id} auction={selectedAuction} session={session} sealedOffer={session && sealedOffers[selectedAuction.id]?.bidderId === session.sessionId ? sealedOffers[selectedAuction.id].amount : undefined} bidValue={bidValues[selectedAuction.id] ?? ""} bidError={bidError} bidInFlight={bidInFlight} onBidChange={(value) => setBidValues((current) => ({ ...current, [selectedAuction.id]: formatNairaInput(value) }))} onSealOffer={() => void sealOffer(selectedAuction)} onClose={() => { setSelectedAuctionId(null); setBidError(""); }} onStartOffer={() => startOffer(selectedAuction)} />}
    {sessionOpen && <AuctionSessionModal audience={sessionIntent} requiresAccessCode={hasConfiguredDeployment} onClose={() => setSessionOpen(false)} onSubmit={handleSession} />}
  </div>;
}
...
function AuctionDrawer({ auction, session, sealedOffer, bidValue, bidError, bidInFlight, onBidChange, onSealOffer, onClose, onStartOffer }: { auction: Auction; session: BidderSession | null; sealedOffer?: bigint; bidValue: string; bidError: string; bidInFlight: boolean; onBidChange: (value: string) => void; onSealOffer: () => void; onClose: () => void; onStartOffer: () => void }) {
...
function BidPanel({ auction, session, sealedOffer, bidValue, bidError, bidInFlight, isOpeningSoon, isClosed, onBidChange, onSealOffer, onStartOffer }: { auction: Auction; session: BidderSession | null; sealedOffer?: bigint; bidValue: string; bidError: string; bidInFlight: boolean; isOpeningSoon: boolean; isClosed: boolean; onBidChange: (value: string) => void; onSealOffer: () => void; onStartOffer: () => void }) {
  ...
  return <section className="bid-panel"><div><span className="eyebrow">Submit your maximum</span><h3>Set the most you are prepared to pay.</h3><p>This is encrypted in your browser before the platform relay submits it. It is never a public current bid.</p></div><label className="bid-label" htmlFor="max-offer">Your confidential Naira maximum<div className="naira-input"><span>{NAIRA_SYMBOL}</span><input id="max-offer" inputMode="numeric" value={bidValue} onChange={(event) => onBidChange(event.target.value)} placeholder={formatNaira(auction.openingBidNaira).slice(1)} aria-describedby="offer-help" /></div></label><p id="offer-help" className="offer-help">Minimum: {formatNaira(auction.openingBidNaira)} / Increments: {formatNaira(auction.bidIncrementNaira)}.</p>{bidError && <p className="bid-error" role="alert"><CircleHelp size={14} />{bidError}</p>}<button className={bidInFlight ? "drawer-primary disabled" : "drawer-primary"} disabled={bidInFlight} onClick={onSealOffer}><LockKeyhole size={17} />{bidInFlight ? "Encrypting and submitting…" : "Submit confidential maximum"}</button></section>;
}

function AuctionSessionModal({ audience, requiresAccessCode, onClose, onSubmit }: { audience: AuctionAudience; requiresAccessCode: boolean; onClose: () => void; onSubmit: (input: { name: string; organisation?: string; contact: string; accessCode?: string }) => void | Promise<void> }) {
